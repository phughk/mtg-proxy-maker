...
fn fetch_card_image(
    ...
    let url = format!(
        ...
        card.set_code.clone().expect("Expected card to have a set code when fetching").to_lowercase(),
        card.collector_number.clone().expect("Expected card to have a collector number when fetching").to_string()
    );
    let resp = client
        .get(&url)
...
fn process_dck_file(
    ...
    let file = File::open(file_path).map_err(|e| format!("Could not open data file: {}", e.to_string()))?;
    ...
    let image_repo = ImageRepository::new(Path::new("image_repository"), None).expect("Expected image repository constructor to work");
    ...
    for (_board, cards) in &deck.cards {
        for card in cards {
            ...
            let front_decoder =
                printpdf::image_crate::codecs::png::PngDecoder::new(&mut front).unwrap();
            let front_img = Image::try_from(front_decoder).unwrap();
            let back_decorder =
                printpdf::image_crate::codecs::png::PngDecoder::new(&mut back).unwrap();
    ...
    doc.save(&mut BufWriter::new(File::create(pdf_file_path).unwrap())).unwrap();

    Ok(())
}

fn main() {
    let file_path = "/Users/hugh/Downloads/mtg-cube-project-halloween/CalebGannonsPoweredSynergyCube.dck";
